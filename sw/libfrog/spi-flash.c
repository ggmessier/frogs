...
uint8_t spiNumFiles;
SpiFlashFileInfo spiFileArray[SPI_FLASH_MAX_NUM_FILES];
char str[30];
...
uint8_t SpiFlashReadStatus(uint8_t registerNum)
{
    ...
    // Clear the collision bit to enable writing.
    SSP1CON1bits.WCOL = 0;

    // A transfer is triggered by writing to SSP1BUF.
    if(registerNum == 0)
      SSP1BUF = 0x05;
    else
      SSP1BUF = 0x35;
